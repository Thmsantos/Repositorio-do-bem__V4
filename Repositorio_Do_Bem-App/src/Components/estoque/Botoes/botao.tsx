...
function Botao() {
  ...
  const [alimentos, setAlimentos] = useState<AlimentoData[]>([]); // Lista de alimentos no estoque

  // Estados para controlar os modais de adicionar alimento e fazer doação
  ...
  async function handleFazerDoacao() {
    if (selectedAlimento) {
      ...
      closeDoacaoModal(); // Fecha o modal após a doação
    } else {
    ...
    <>
      <div className="button-container">
        ...
        <div className="button-group">
          <button className="botoes" onClick={openAddModal}>+ Add alimento</button>
          <button className="botoes" onClick={openDoacaoModal}>- Fazer doação</button>
        </div>
      </div>
